  ...
  const fetchProductInfo = async (barcodeNumber: string) => {
    ...
    try {
      ...
      if (data.I2570.RESULT.CODE !== 'INFO-000') {
        setError(`API 에러: ${data.I2570.RESULT.MSG || '알 수 없는 에러'}`);
        setStep('scanning');
        return;
      }

      if (!data.I2570.row || data.I2570.row.length === 0) {
        setError('해당 바코드에 대한 제품 정보를 찾을 수 없습니다.');
        setStep('scanning');
        return;
      }
      ...
      try {
        ...
        setError('영양 정보를 가져오는 중 오류가 발생했습니다.');
        console.error('Nutrition API Error:', nutritionError);
        setStep('scanning');
      }
    } catch (err) {
      console.error('API Error:', err);
      setError('제품 정보를 가져오는 중 오류가 발생했습니다. 다시 시도해주세요.');
      setStep('scanning');
    } finally {
      setIsLoading(false);
    }
